...
Checksum files are listed after the two downloads on purpose.
...
Extract the zip, then open FlexWeek.exe. If Windows shows "Windows protected your PC", choose More info, then Run anyway. FlexWeek is not code-signed yet. Unicode text support (ICU) is part of Windows 10 version 1809 and later.
...
## Checksums
Optional. `FlexWeek-Windows-x64.zip.sha256` and `FlexWeek-Linux-x86_64.tar.gz.sha256` on this page.
