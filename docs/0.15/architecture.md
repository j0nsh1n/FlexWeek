...
class MissionView(LayoutView):
    def __init__(self, parent=None, *, hand=None):
        ...
        self.week = HoursCanvas(self.hand, MissionPainter(tokens), lanes, header=24)
        self.scroll = HoursScroll(self.week, Scale("mission.week", (32, 48, 64, 96), 48),
                                  lambda px: 24 * px + 16, name="missionWeek", gutter=72,
                                  axis=Axis.ACROSS)
    ...
    # render(scene): self.week.set_week(scene.week.occurrences, scene.today, scene.minute)
...
- The pointer resting 300 ms within 36 pixels of a scroll area's edge scrolls it; passing through
  does not.
- A long block's name stays in sight at the top of what shows, placed from the viewport, not from
  the part being repainted.
...
- Chips for weeks other than the open one need the month reply's per-date blocks
  (`handoffs/grok-month-blocks.md`); weeks left with unsaved changes must then be drawn from those
  changes, as the open week is.
